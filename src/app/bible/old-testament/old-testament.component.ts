...
@Component({
  selector: 'old-testament',
  templateUrl: './old-testament.component.html',
  styleUrls: ['./old-testament.component.scss']
})
export class OldTestamentComponent implements OnInit, OnDestroy, AfterViewInit {
  @Input('bible') oldTestamentBible: BibleBook[];
  ...
  private sortBibleBooks(): BibleBook[] {
    return this.oldTestamentBible.sort((bibleBookA: BibleBook, bibleBookB: BibleBook) => (bibleBookA.no > bibleBookB.no) ? 1 : -1)
  }
   ...
   * @returns the first result matching the param book passed
    */
  getBibleBook(book: string = 'Genesis'): BibleBook {
  ...
  matSelectInputSearchConfig() {
    this.filteredBibleBooks.next(this.oldTestamentBible.slice());
  ...
  filterBooks() {
    ...
    this.filteredBibleBooks.next(
      this.oldTestamentBible.filter(book => book.book.toLowerCase().indexOf(search) > -1)
    );
  }
  ...
  protected setInitialValue() {
    this.filterFormGroup.get('filterBibleByBook').setValue('Genesis');
    this.filterFormGroup.get('filterBibleByChapter').setValue(1);
